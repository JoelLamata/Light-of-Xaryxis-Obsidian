---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/bgdia
- ttrpg-cli/monster/cr/1-2
- ttrpg-cli/monster/size/medium
- ttrpg-cli/monster/type/humanoid/human
aliases: ["Thavius Kreeg"]
---
# Thavius Kreeg
*Source: Baldur's Gate: Descent Into Avernus p. 42*  

```ad-statblock
title: Thavius Kreeg
![](https://raw.githubusercontent.com/5etools-mirror-2/5etools-img/main/bestiary/tokens/BGDIA/Thavius%20Kreeg.webp#token)
*Medium humanoid (human), Lawful Evil*

- **Armor Class** 10 
- **Hit Points** 40 (`9d8`)
- **Speed** 30 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|12 (+1)|10 (+0)|11 (+0)|15 (+2)|18 (+4)|16 (+3)|

- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** Deception +5, Medicine +6, Persuasion +5, Religion +4
- **Senses** passive Perception 14
- **Languages** Abyssal, Common, Infernal
- **Challenge** 1/2

## Traits

***Shadow of Guilt.*** Thavius's shadow is that of a pudgy, horned devil with small wings.

## Actions

***Mace.*** *Melee Weapon Attack:* `dice: d20+3` (+3 to hit), reach 5 ft., one target. *Hit:* `dice: 1d6 + 1|avg` (`1d6 + 1`) bludgeoning damage.
```
^statblock