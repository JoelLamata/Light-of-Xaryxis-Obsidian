...
***Divine Eminence.*** As a bonus action, Refrum can expend a spell slot to cause its melee weapon attacks to magically deal an extra `dice: 3d6|avg` (`3d6`) radiant damage to a target on a hit. This benefit lasts until the end of the turn. If Refrum expends a spell slot of 2nd level or higher, the extra damage increases by `dice: 1d6|avg` (`1d6`) for each level above 1st.

***Spellcasting.*** Refrum is a 5th-level spellcaster. Its spellcasting ability is Wisdom (spell save DC 13, `dice: d20+5` (+5 to hit) with spell attacks). Refrum has the following cleric spells prepared:
