...
```ad-statblock
title: Giff Shipmate
![](https://raw.githubusercontent.com/5etools-mirror-2/5etools-img/main/bestiary/tokens/BAM/Giff%20Shipmate.webp#token)
...
***Steady as She Goes.*** On the deck of a ship, the giff has advantage on ability checks and saving throws made against effects that would knock it [prone](/compendium/rules/conditions.md#prone) or shove it overboard.
...
***Force Grenade.*** The giff throws a grenade up to 60 feet, and the grenade explodes in a 20-foot-radius sphere. Each creature in that area must make a DC 15 Dexterity saving throw, taking `dice: 5d6|avg` (`5d6`) force damage on a failed save, or half as much damage on a successful one. After the giff throws the grenade, roll a `dice: d6|avg` (`d6`); on a roll of 4 or lower, the giff has no more grenades to throw.
```
^statblock