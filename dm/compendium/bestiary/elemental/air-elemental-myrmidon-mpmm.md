...
***Lightning Strike (Recharge 6).*** The myrmidon makes one Flail attack. On a hit, the target takes an extra `dice: 4d8|avg` (`4d8`) lightning damage, and the target must succeed on a DC 13 Constitution saving throw or be [stunned](/compendium/rules/conditions.md#stunned) until the end of the myrmidon's next turn.
```
^statblock